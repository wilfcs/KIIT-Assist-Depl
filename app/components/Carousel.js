...
export default function Carousel({ items, carouselHeight, itemHeight }) {
  const duration = 5000;
  ...
  return (
    <div className="w-full max-w-5xl mx-auto text-center ${carouselHeight} bgW">
      {/* Item image */}
      <div className="transition-all duration-150 delay-300 ease-in-out">
        <div className="relative flex flex-col" ref={itemsRef}>
          {items.map((item, index) => (
            ...
            >
              <Image
                className={`rounded-xl ${itemHeight}`}
                src={item.img}
                width={1024}
                height={576}
                alt={item.desc}
